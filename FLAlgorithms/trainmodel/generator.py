...
class Generator(nn.Module):
    def __init__(self, dataset='mnist', model='cnn', embedding=False, latent_layer_idx=-1):
        ...
        self.init_loss_fn()
        self.build_network()
        self.build_network_full()
   
    def build_network_full(self):
        
        self.z_dim = self.noise_dim + self.n_class
    ...
    def build_network(self):
        ...
        ### FC modules ####
        '''
        fc layrers : build layers according to fc_config
        '''
    ...
    def forward(self, labels, latent_layer_idx=-1, verbose=True):
        ...
        if latent_layer_idx == -1 :
            ...
            if self.embedding: # embedded dense vector
                ...
                y_input = torch.FloatTensor(batch_size, self.n_class) # (32, 10)
                y_input.zero_()
                #labels = labels.view
                labels_int64 = labels.type(torch.LongTensor)
                y_input.scatter_(1, labels_int64.view(-1,1), 1) # (32, 10) one-hot label


            z = torch.cat((eps, y_input), dim=1) # (32, 10 + noise_dim)
            z = z.to(torch.device('cuda:3'))
            ### FC layers 
            for layer in self.fc_layers:
                z = layer(z)

            # representation layer: linear projection: -> latent dim (dim of the output feature)
            # z: (32, 32)
        ...
        # raw image: 
            ...
            # generation: 
            out = self.l1(z.to(torch.device('cuda:3'))) # input of l1: (batch size, self.z_dim), a batch of noise
